...
similarity relation is not transitive. For example, if the words a and b are similar, and 
...
public class Q734_SentenceSimilarity {
    public static void main(String[] args) {
        Solution solution = new Q734_SentenceSimilarity().new Solution();
    }
    ...
    public boolean areSentencesSimilar(String[] sentence1, String[] sentence2, List<List<String>> similarPairs) {
        ...
        for (int i = 0; i < sentence1.length; i++) {
            if (Objects.equals(sentence1[i], sentence2[i])
                    || set.contains(sentence1[i].concat("-").concat(sentence2[i]))
                    || set.contains(sentence2[i].concat("-").concat(sentence1[i]))) {
                continue;
            }
            return false;
        }
